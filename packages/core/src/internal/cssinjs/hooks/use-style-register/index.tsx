import canUseDom from '@/components/_util/can-use-dom';
import { removeCSS, updateCSS } from '@/components/_util/dynamicCss';
import { StencilVode } from '@/components/_util/type';
...
export type CSSPropertiesWithMultiValues = {
  [K in keyof CSSProperties]:
    | CSSProperties[K]
    | Extract<CSSProperties[K], string | number>[]
    | {
        [SKIP_CHECK]: boolean;
        [MULTI_VALUE]?: boolean;
        value: CSSProperties[K] | Extract<CSSProperties[K], string | number>[];
      };
};
...
export interface CSSObject extends Omit<CSSPropertiesWithMultiValues, keyof CSSPseudos>, CSSPseudos {
  [key: string]: CSSInterpolation | CSSProperties[keyof CSSProperties];
}
...
export default function useStyleRegister(
  ...
  >(
    ...
    () => {
      ...
      const { hashPriority, container, transformers, linters, cache } = styleContext;
...
export function extractStyle(cache: Cache, plain = false) {
  ...
  function toStyleStr(style: string, tokenKey?: string, styleId?: string, customizeAttrs: Record<string, string> = {}) {
